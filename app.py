...
# 设置页面配置
st.set_page_config(page_title="SmartCampus — CUHK校园生活助手", layout="wide", page_icon="🎓")
...
if "agent_network" not in st.session_state:
    ...
    st.session_state.agent_urls = {
        ...
        "BookingAssistant": "http://localhost:5007"
    }
    # 初始化网络
    network = AgentNetwork(name="CUHK Campus Assistant Network")
    network.add("CourseQueryAssistant", "http://localhost:5005")
    network.add("FacilityQueryAssistant", "http://localhost:5006")
    network.add("BookingAssistant", "http://localhost:5007")
...
def intent_agent(user_input):
    # 创建意图识别链：提示模板 + LLM
    chain = SmartCampusPrompts.intent_prompt() | st.session_state.llm
...
# 主界面布局
st.title("🎓 SmartCampus — CUHK校园生活智能助手")
st.markdown("欢迎使用CUHK校园服务！查询课程、找自习室、预约图书馆座位、发现校园活动，一句话搞定。")
...
with col1:
    ...
    # 输入框
    if prompt := st.chat_input("请输入您的问题（如：CSCI2100的上课时间 / YIA明天有哪些自习室 / 最近有什么讲座）..."):
        # 显示用户消息
            ...
            try:
                ...
                if "out_of_scope" in intents:
                    ...
                    for intent in intents:
                        ...
                        # 根据意图确定代理名称
                        if intent == "course":
                            agent_name = "CourseQueryAssistant"
                        elif intent in ["study_room", "library_seat", "campus_event"]:
                            agent_name = "FacilityQueryAssistant"
                        elif intent == "booking":
...
# 页脚
st.markdown("---")
st.markdown('<div class="footer">Powered by CUHK CS | SmartCampus — 基于A2A的校园生活助手 v2.0</div>', unsafe_allow_html=True)
